...
async def yardim(ctx):
    ...
    embed.add_field(name="🤖 TuffAI Sohbet", value=f"`!sor <soru>` or type directly in `#{AI_SOHBET_KANAL_ADI}` channel", inline=False)
...
async def sor(ctx, *, soru):
    async with ctx.typing():
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f"https://api.lolhuman.xyz/api/openai?apikey=free&text={soru}") as resp:
                    cevap = await resp.text()
                    embed = discord.Embed(
                        title="🤖 TuffAI Cevabı",
                        description=cevap[:4000],
            ...
            await ctx.send("❌ TuffAI şu anda y
