...
async def userinfo(ctx, uye: discord.Member = None):
    ...
    embed.add_field(name="ID", value=uye.id, inline=False)
    embed.add_field(name="Katılım", value=uye.joined_at.strftime("%d/%m/%Y"), inline=True)
    embed.add_field(name="Hesap", value=uye.created_at.strftime("%d/%m/%Y"), inline=True)
    await ctx.send(embed=embed)

@bot.command()
async def botinfo(ctx):
    await ctx.send(f"🤖 Sunucu: {len(bot.guilds)} | Ping: {round(bot.latency*1000)}ms")
...
async def slowmode(ctx, saniye:int):
    ...
    await ctx.send(f"🐢 Slowmode {saniye}s")

@bot.command(name="8ball")
async def eightball(ctx, *, soru):
    await ctx.send(random.choice(["Evet","Hayır","Belki","Kesinlikle","Sanmam"]))

@bot.command()
async def ship(ctx, uye1: discord.Member, uye2: discord.Member):
    await ctx.send(f"❤️ Uyum: %{random.randint(1,100)}")
