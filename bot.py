...
async def slot(ctx):
    ...
    embed_start.set_footer(text="Dönüyor...")
    msg = await ctx.send(embed=embed_start)
    await slot_animasyon(msg, s1, s2, s3)

    # Sonuç hesapla
    combo = (s1, s2, s3)
    if combo in SLOT_ÖDÜLLER:
        ad, çarpan = SLOT_ÖDÜLLER[combo]
        kazanç = SLOT_BAHIS * çarpan
        slot_bakiye[uid] += kazanç
        renk = discord.Color.gold()
        sonuç_text = f"🎉 **{ad}** +{kazanç} 🪙 kazandın!"
    elif s1 == s2 or s2 == s3 or s1 == s3:
        kazanç = SLOT_BAHIS // 2
        slot_bakiye[uid] += kazanç
        renk = discord.Color.green()
        sonuç_text = f"🟡 İki eşleşme! +{kazanç} 🪙"
    else:
        kazanç = 0
        renk = discord.Color.red()
        sonuç_text = f"❌ Kaybettin. -{SLOT_BAHIS} 🪙"

    embed_son = discord.Embed(title="🎰 Slot Makinesi", color=renk)
    embed_son.add_field(
        name="Sonuç",
        value=f"┌─────────────┐\n│ {s1}  {s2}  {s3} │\n└─────────────┘",
        inline=False
    )
    embed_son.add_field(name="Sonuç", value=sonuç_text, inline=False)
    embed_son.add_field(name="💰 Bakiye", value=f"{slot_bakiye[uid]} 🪙", inline=True)
    embed_son.add_field(name="🎰 Bahis", value=f"{SLOT_BAHIS} 🪙", inline=True)
    embed_son.set_footer(text=f"Oyuncu: {ctx.author.display_name}")
    await msg.edit(embed=embed_son)

@bot.command(name="slotdaily", aliases=["slotbonus"])
async def slotdaily(ctx):
    """Günlük 50 🪙 bonus al"""
    uid = ctx.author.id
    if uid not in slot_bakiye:
        slot_bakiye[uid] = SLOT_BAŞLANGIÇ
    ...
    embed = discord.Embed(
        title="🎁 Günlük Bonus",
        description=f"**+50 🪙** eklendi!\nYeni bakiye: **{slot_bakiye[uid]} 🪙**",
        color=discord.Color.green()
    )
    await ctx.send(embed=embed)

@bot.command(name="slotbakiye", aliases=["bakiye", "coins"])
async def slotbakiye_cmd(ctx, üye: discord.Member = None):
    """Bakiyeni göster"""
    ...
    embed = discord.Embed(
        title="💰 Bakiye",
        description=f"{üye.mention}: **{slot_bakiye[uid]} 🪙**",
