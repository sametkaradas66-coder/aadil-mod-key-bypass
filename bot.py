...
PREFIX = "!"
HOSGELDIN_KANAL = "genel"
AI_SOHBET_KANAL_ADI = "tuffai-sohbet"
...
async def on_message(message):
    ...
    if isinstance(message.channel, discord.TextChannel) and message.channel.name == AI_SOHBET_KANAL_ADI:
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(f"https://api.lolhuman.xyz/api/openai?apikey=free&text={message.content}") as resp:
                        cevap = await resp.text()
                        embed = discord.Embed(
                            title="🤖 TuffAI Cevabı",
                            description=cevap[:4000],
                            color=discord.Color.blurple()
                        )
                        embed.set_footer(text=f"Soran: {message.author}")
                        await message.reply(embed=embed)
                        return
            except:
                await message.reply("❌ TuffAI şu anda yanıt veremiyor, lütfen daha sonra tekrar deneyin.")
                return
...
async def yardim(ctx):
    ...
    embed.add_field(name="🤖 TuffAI Sohbet", value=f"`!sor <soru>` veya `#{AI_SOHBET_KANAL_ADI}` kanalına doğrudan yazın", inline=False)
...
async def mute(ctx, üye: discord.Member, dakika: int = 10, *, sebep="Sebep belirtilmedi"):
    süre = discord.utils.utcnow() + datetime.timedelta(minutes=dakika)
