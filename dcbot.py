...
async def invites(interaction: discord.Interaction, uye: discord.Member=None):
    uye = uye or interaction.user
    await interaction.response.send_message(
        embed=discord.Embed(
            title="📨 Davet Bilgileri",
            description=f"{uye.mention} için davet istatistikleri yakında geliştirilecek.",
            color=discord.Color.blurple()
        )
    )
...
@bot.tree.command(name="ticketkapat")
